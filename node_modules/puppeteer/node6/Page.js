...
class Page extends EventEmitter {
   ...
   * @return {!Promise<!Page>}
   */
  static /* async */ create(client, ignoreHTTPSErrors, screenshotTaskQueue) {return (fn => {
   ...
   * @param {boolean} value
   */
  /* async */ setRequestInterceptionEnabled(value) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
   ...
   * @param {string} url
   */
  /* async */ addScriptTag(url) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
   ...
   */
  /* async */ injectFile(filePath) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
  ...
  return new Promise((resolve, reject) => {
    ...
    (yield this._client.send('Runtime.evaluate', { expression, returnByValue: true }));
  ...
  return new Promise((resolve, reject) => {
    ...
    const values = (yield Promise.all(event.args.map(arg => helper.serializeRemoteObject(this._client, arg))));
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      (yield this._client.send('Page.navigate', {url, referrer}));
    } catch (e) {
      watcher.cancel();
      throw e;
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    const responses = new Map();
    const listener = helper.addEventListener(this._networkManager, NetworkManager.Events.Response, response => responses.set(response.url, response));
    const error = (yield watcher.waitForNavigation());
    helper.removeEventListeners([listener]);
    if (error)
      throw error;
    return responses.get(this.mainFrame().url()) || null;
  ...
  return new Promise((resolve, reject) => {
    ...
    (yield this._client.send('Target.activateTarget', {targetId: this._client.targetId()}));
  ...
  return new Promise((resolve, reject) => {
    ...
    const handle = (yield this.$(selector));
    console.assert(handle, 'No node found for selector: ' + selector);
    (yield this.evaluate(element => element.focus(), handle));
    (yield handle.dispose());
  });}
